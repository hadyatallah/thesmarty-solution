...
export const commandCenter={
 ...
 mount({call,session}){const root=document.getElementById('ccControls');if(!root)return;
  const ms=async(path)=>{const token=typeof session==='function'?session():'';const r=await fetch(path,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({session:token})});let data;try{data=await r.json()}catch{throw Error('Unexpected Microsoft connection response.');}if(!r.ok||data.ok===false)throw Error(data.error||'Microsoft connection unavailable.');return data;};
