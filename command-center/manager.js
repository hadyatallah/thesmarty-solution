...
export class Manager {
 ...
 async run(command,now=new Date()) {
  ...
  else if(/prepare.*(?:email|whatsapp|follow.up|introduction)/i.test(q))add('Communications',()=>{const match=q.match(/\bfor\s+(.+?)\s+about\s+(.+)$/i);if(!match||!this.growth)return {status:'needs_context',message:'Specify the exact company and purpose, for example: Prepare an email for [company] about [purpose]. Nothing has been sent.'};const company=this.crm.lookup(match[1]);if(!company.record)return company;return this.growth.prepareDraft({companyId:company.record.id,channel:/whatsapp/i.test(q)?'whatsapp':'email',purpose:match[2],followUp:/follow.up/i.test(q)});});
