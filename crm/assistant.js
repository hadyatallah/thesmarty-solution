...
function assistantWorkspaceHtml(){
  loadAiState();
  return'<section class="panel assistant-panel"><div class="row"><div><h3>CRM Assistant</h3><p class="muted">Ask, explain, search, paste an email, or tell me what changed. Use normal language.</p></div><span class="assistant-status">'+(state.aiEnabled?'AI connected':'AI unavailable')+'</span></div><form id="aiForm"><label><span>Optional context</span><input id="aiContextSearch" type="search" autocomplete="off" placeholder="Pin a record only if you want to…" oninput="assistantContextSearch(this.value)"></label><div id="aiContextResults">'+(aiContextSelection.id?'<span class="assistant-context-pill">'+esc(aiContextSelection.label)+' <button type="button" onclick="clearAssistantContext()" style="min-height:auto;padding:1px 6px">×</button></span>':'<p class="muted" style="margin:8px 0 0">No fixed context. I will infer the relevant records.</p>')+'</div><label style="margin-top:16px"><span>What do you want to do?</span><textarea class="assistant-input" id="aiQuestion" maxlength="12000" placeholder="Type naturally, paste an email, ask what a CRM status means, or describe what happened…" required></textarea></label><div class="actions"><button type="button" onclick="clearAiConversation()">Clear conversation</button><button type="submit" class="primary" '+(!state.aiEnabled?'disabled':'')+'>Send</button></div></form><div id="aiAnswer" class="assistant-response" role="status">'+aiConversation.slice(-4).map(function(m){return'<div class="assistant-message '+(m.role==='user'?'user':'')+'"><strong>'+(m.role==='user'?'You':'Assistant')+'</strong><br>'+esc(m.text)+'</div>';}).join('')+'</div>'+assistantHistoryHtml()+'</section>';
}
...
function assistant(){el('main').innerHTML='<h2>CRM Assistant</h2><p class="muted">The same assistant used on your landing page, with more room for longer CRM work.</p>'+assistantWorkspaceHtml();wireAssistantForm();}
