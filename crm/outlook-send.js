...
export async function prepareOutlookSend({session,message,dialog}){
 ...
 dialog.querySelector('[data-send-email]').addEventListener('click',async e=>{
  ...
  try{
   ...
   status.textContent=sd.status==='succeeded'?'Sent and reconciled with Microsoft 365 at '+sd.receipt.sentDateTime+'.':'Accepted by Microsoft 365 at '+sd.receipt.acceptedAt+'. Sent Items reconciliation is still pending. Do not resend.';sendButton.remove();const closeButton=dialog.querySelector('[data-close]');if(closeButton){closeButton.textContent='Close';closeButton.focus();}if(sd.status==='succeeded')setTimeout(()=>{if(dialog.open)dialog.close();},2200);
