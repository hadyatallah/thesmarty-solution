...
const ask=async q=>{byId('question').value=q;byId('response').textContent='Checking synthetic records…';try{byId('response').innerHTML=await commandCenter.answer(q,sandbox.data,{call:sandbox.call})||'<p>This request needs the live AI interpreter, which is disconnected in this test. Use a sample command below.</p>';}catch(e){byId('response').textContent=e.message;}};
